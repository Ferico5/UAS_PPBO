...
class ComplaintAction {
    ...
    public function __construct($conn, $complaintId, $email) {
        $this->conn = $conn;
        $this->complaintId = $complaintId;
        $this->email = $email;
        $this->setRegistrationNo();
        $this->setFullName();
        $this->setComplaintDetails();
    }

    private function setRegistrationNo() {
        $query = mysqli_query($this->conn, "SELECT registration_no FROM registration WHERE email = '$this->email'");
        if ($query) {
            $this->registrationNo = mysqli_fetch_assoc($query)['registration_no'];
        } else {
    ...
    private function setFullName() {
        ...
        if ($queryName) {
            ...
            $this->fullName = $nameResult['full_name'];
        } else {
            die("Query failed: " . mysqli_error($this->conn));
        }
    }

    private function setComplaintDetails() {
        $query3 = mysqli_query($this->conn, "SELECT 
                                                ...
                                                WHERE register_complaint.register_complaint_id = '$this->complaintId'
                                                ORDER BY complaint_action.posting_date DESC
                                                LIMIT 1");
        if ($query3) {
            $this->complaintDetails = mysqli_fetch_all($query3, MYSQLI_ASSOC);
        } else {
            die("Query failed: " . mysqli_error($this->conn));
        }
    }
...
$complaintAction = new ComplaintAction($conn, $complaintId, $email);
$fullName = $complaintAction->getFullName();
$complaintDetails = $complaintAction->getComplaintDetails();
                ...
                <div class="box">
                    ...
                    <div class="boxinfo">
                        <table border="1" cellspacing="0">
                            ...
                            <tbody>
                                <?php foreach ($complaintDetails as $row) : ?>
                                <tr>
