...
class RoomInfoUpdater {
    ...
    public function getAdminName($email) {
        $query = "SELECT username FROM admin_login WHERE admin_email = '$email'";
    ...
    public function getRoomInfo($room_no) {
    ...
    public function updateRoomInfo($room_no, $seater, $fees_per_month, $remaining_seater) {
        $room_no = mysqli_real_escape_string($this->conn, $room_no);
        $seater = mysqli_real_escape_string($this->conn, $seater);
        $fees_per_month = mysqli_real_escape_string($this->conn, $fees_per_month);
        $remaining_seater = mysqli_real_escape_string($this->conn, $remaining_seater);

        $updateQuery = "UPDATE room_info SET
                        seater = '$seater',
                        fees_per_month = '$fees_per_month',
                        remaining_seater = '$remaining_seater'
                        WHERE room_no = '$room_no'";

        if (mysqli_query($this->conn, $updateQuery)) {
            header("Location: roominfo.php");
        } else {
            echo "Error updating room details: " . mysqli_error($this->conn);
        }
    }
}

$roomUpdater = new RoomInfoUpdater($conn);

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $room_no = $_POST['room_no'];
    $seater = $_POST['seater'];
    $fees_per_month = $_POST['fees_per_month'];
    $remaining_seater = $_POST['remaining_seater'];

    $roomUpdater->updateRoomInfo($room_no, $seater, $fees_per_month, $remaining_seater);
}

$fullName = $roomUpdater->getAdminName($_SESSION['email']);
...
$result = $roomUpdater->getRoomInfo($room_no);
            ...
            <div class="container">
                <div class="box">
                    <form class="form" method="post" action="">
                        <div class="fillform">
                            <p>Room No : <input type="text" id="room_no" name="room_no" value="<?php echo htmlspecialchars($result['room_no']); ?>" readonly>
                            <p>Seater : <input type="number" id="seater" name="seater" value="<?php echo htmlspecialchars($result['seater']); ?>" required></p>
                            <p>Fees Per Month : <input type="number" id="fees_per_month" name="fees_per_month" value="<?php echo htmlspecialchars($result['fees_per_month']); ?>" required></p>
                            <p>Remaining Seater : <input type="number" id="remaining_seater" name="remaining_seater" value="<?php echo htmlspecialchars($result['remaining_seater']); ?>" required></p>
