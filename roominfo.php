...
class RoomInfoPage {
    ...
    public function fetchRoomInfo() {
        $query2 = mysqli_query($this->conn, "SELECT
                                            room_no,
                                            seater, 
                                            fees_per_month,
                                            remaining_seater
        ...
        return mysqli_fetch_all($query2, MYSQLI_ASSOC);
    }

    public function deleteRoom($room_no) {
        $room_no = mysqli_real_escape_string($this->conn, $room_no);
        $deleteQuery = "DELETE FROM room_info WHERE room_no = '$room_no'";
...
$roomInfoPage = new RoomInfoPage($conn);

$result = $roomInfoPage->fetchRoomInfo();

// hapus kamar jika button di klik
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    $room_no = $_POST['room_no'];
    if ($roomInfoPage->deleteRoom($room_no)) {
        header('Location: roominfo.php');
    }
}

$fullName = $roomInfoPage->getAdminName($getEmail);
                        ...
                        <table border="1" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td id="center"><?= $row["seater"]; ?></td>
                                    <td id="center"><?= $row["fees_per_month"]; ?></td>
                                    <td id="center"><?= $row["remaining_seater"]; ?></td>
                                    <td id="center"><a href="roominfoupdate.php?room_no=<?php echo $row['room_no']; ?>"><button>Update</button></a></td>
                                    <td id="center">
