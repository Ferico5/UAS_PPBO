...
class ComplaintActionAdmin {
    ...
    public function fetchComplaintDetails($id) {
        $query = "SELECT 
                    register_complaint.register_complaint_id,
                    ...
                    complaint_action.posting_date
                    FROM register_complaint
                    LEFT JOIN complaint_action ON register_complaint.register_complaint_id = complaint_action.register_complaint_id
                    WHERE register_complaint.register_complaint_id = '$id'
                    ORDER BY complaint_action.posting_date DESC
                    LIMIT 1";
    ...
    public function updateComplaint($id, $status, $remark) {
        $timezone = new DateTimeZone('Asia/Jakarta');
        $time = new DateTime('now', $timezone);
        $date = $time->format('Y-m-d H:i:s');

        $updateQuery = "UPDATE register_complaint SET complaint_status='$status' WHERE register_complaint_id='$id'";
        $insertRemark = "INSERT INTO complaint_action (register_complaint_id, complaint_remark, posting_date) VALUES ('$id', '$remark', '$date')";

        if (mysqli_query($this->conn, $updateQuery) && mysqli_query($this->conn, $insertRemark)) {
            return true;
        } else {
            return false;
        }
...
$complaintAdmin = new ComplaintActionAdmin($conn);

// dapatkan ID dari URL
$id = isset($_GET['id']) ? $_GET['id'] : null;

$result = $complaintAdmin->fetchComplaintDetails($id);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $status = $_POST['status'];
    $remark = $_POST['remark'];

    if ($complaintAdmin->updateComplaint($id, $status, $remark)) {
...
$fullName = $complaintAdmin->fetchAdminName($_SESSION["email"]);
...
<body>
    ...
    <div class="main">
        ...
        <div class="maincontent">
            ...
            <p id="userlogin">Complaint Action</p>
            <hr>
                    ...
                    <div class="boxinfo">
                        <form action="" method="post">
                            <table border="1" cellspacing="0">
                                <thead>
                                    <tr>
                                        <th colspan = "4" class="blue">Complaint Related Info</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($result as $row) : ?>
                                    <tr>
                                        <td class="bold">Complaint Number :</td>
                                        <td><?= $row["register_complaint_id"]; ?></td>
                                    ...
                                    <tr>
                                        <td class="bold">Complaint Remark :</td>
                                        <td colspan="3"><?= $row["complaint_remark"]; ?></td>
                                    </tr>
                                    <tr>
                                        <td class="bold">Posting Date :</td>
                                        <td colspan="3"><?= $row["posting_date"]; ?></td>
                                    </tr>
                                    <tr>
                                        <td colspan="4"><button type="submit" id="update">Update</button></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                                ...
                                <div class="modal-body">
                                    <form method="post">
                                        ...
                                        <div class="right">
                                            ...
                                            <input type="text" id="remarkText" name="remark">
                                        </div>
    ...
    <script src="complaintActionAdmin.js"></script>
</body>
</html>
