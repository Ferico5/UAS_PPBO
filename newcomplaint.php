...
class NewComplaint {
    ...
    public function getAdminName($email) {
        ...
        $sql = "SELECT username FROM admin_login WHERE admin_email = '$email'";
    ...
    public function getNewComplaints() {
        $status = mysqli_real_escape_string($this->conn, "unprocessed");

        $query = "SELECT register_complaint_id, room_no, complaint_type, complaint_status, complaint_date
...
$newComplaint = new NewComplaint($conn);

// ambil admin email dari sesi
$email = $_SESSION["email"];

$fullName = $newComplaint->getAdminName($email);

$result = $newComplaint->getNewComplaints();
...
<body>
    <!-- header -->
    <header>
        <a href="dashboardadmnin.php">Hostel Management System</a>
        <div class="user-info">
