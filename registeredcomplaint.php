...
class ComplaintSystem {
    ...
    // Method untuk memeriksa apakah admin sudah login
    ...
    // Method untuk mendapatkan nama admin
    public function getAdminName() {
        $getEmail = $_SESSION["email"];
        $queryName = "SELECT username FROM admin_login WHERE admin_email = '" . mysqli_real_escape_string($this->conn, $getEmail) . "'";
    ...
    // Method untuk mendapatkan daftar keluhan yang terdaftar
    public function getRegisteredComplaints() {
        $queryComplaints = "SELECT register_complaint_id, room_no, complaint_type, complaint_status, complaint_date FROM register_complaint";
        $result = mysqli_query($this->conn, $queryComplaints);
        if (!$result) {
            die("Query failed: " . mysqli_error($this->conn));
        }
...
// Penggunaan kelas ComplaintSystem untuk menampilkan data di halaman HTML
$complaintSystem = new ComplaintSystem();
$complaintSystem->checkAdminLogin();
$fullName = $complaintSystem->getAdminName();
$complaints = $complaintSystem->getRegisteredComplaints();
$complaintSystem->closeConnection();
...
<body>
    ...
    <div class="main">
        ...
        <div class="maincontent">
            ...
            <p id="userlogin">Registered Complaints</p>
            <hr>
                    ...
                    <div class="boxinfo">
                        <table border="1" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td id="center"><?= $i; ?></td>
                                    <td id="center"><?= $row["room_no"]; ?></td>
                                    <td><?= $row["complaint_type"]; ?></td>
                                    <td><?= $row["complaint_status"]; ?></td>
                                    <td><?= $row["complaint_date"]; ?></td>
                                    <td id="center"><a href="complaintactionadmin.php?id=<?php echo $row['register_complaint_id']; ?>"><button>Action</button></a></td>
                                </tr>
                                <?php $i++; ?>
                                <?php endforeach; ?>
                            </tbody>
